...
const Cart = ({ onBack, onCheckout }) => {
  ...
  const handleUpdateQuantity = async (id, currentQty, change) => {
    ...
    try {
      ...
      if (error) throw error;
      window.dispatchEvent(new Event('cartUpdated')); 
    } catch (error) { toast.error("Failed to update quantity"); fetchCart(); }
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 animate-in slide-in-from-right duration-300 pb-28">
      <div className="sticky top-0 z-50 flex items-center bg-white px-4 py-4 shadow-sm border-b border-gray-100">
        <button onClick={onBack} className="p-2 -ml-2 rounded-full hover:bg-gray-100 active:scale-95 transition-all"><ArrowLeft className="h-6 w-6 text-gray-800" /></button>
        <h1 className="ml-2 text-lg font-black text-gray-900">Your Cart</h1>
      </div>
      ...
      {cartItems.length === 0 ? (
        <div className="flex flex-1 flex-col items-center justify-center p-6 text-center">
          ...
          <button onClick={onBack} className="rounded-xl bg-[#e31818] px-8 py-3.5 font-bold text-white shadow-lg active:scale-95 transition-all">Continue Shopping</button>
        </div>
      ) : (
        <div className="flex flex-col p-4">
          <div className="flex flex-col gap-4">
            {cartItems.map(item => {
              ...
              return (
                <div key={item.id} className="flex overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm p-3 gap-3">
                  <div className="w-24 h-24 bg-gray-100 flex items-center justify-center rounded-lg overflow-hidden flex-shrink-0">
                  ...
                  <div className="flex flex-1 flex-col justify-between py-1">
                    <div className="flex justify-between items-start">
                      <div className="pr-2">
                        ...
                        {isGift && <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mt-1 mb-2">{item.selected_option?.label}</p>}
                        
                        <div className={`flex items-center gap-3 bg-gray-50 rounded-full px-2 py-1 border border-gray-200 w-fit ${!isGift ? 'mt-2' : ''}`}>
                          <button onClick={() => handleUpdateQuantity(item.id, itemQty, -1)} disabled={itemQty <= 1} className="w-6 h-6 flex items-center justify-center rounded-full bg-white text-gray-600 shadow-sm border border-gray-200 font-bold disabled:opacity-30 active:scale-95 transition-all">-</button>
                      ...
                      <button onClick={() => handleRemoveItem(item.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 className="h-5 w-5" /></button>
                    </div>
      ...
      {cartItems.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white border-t border-gray-200 p-4 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-50">
